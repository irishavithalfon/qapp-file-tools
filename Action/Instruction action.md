...
---
#LANGUAGE POLICY
The final Q-App (.qapp or .zip) must be physically generated by Action:
1) createZipFromUploadedFiles first
2) If Action packaging fails or lacks valid archive evidence -> Python packaging

Do not return HARD STOP before attempting createZipFromUploadedFiles, unless required inputs are missing.
...
For uploaded files:
- Required: uploadedFiles with download_link + targetPath for every file
- Do NOT infer target path from file name
- Do NOT use openaiFileIdRefs for package creation
...
3) uploadedFiles.targetPath values
...
- Use openaiFileIdRefs or pathOverrides for package creation
- Add extra files
- Miss files
